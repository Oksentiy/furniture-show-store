...
export const BasketContainer: React.FC<BasketContainerProps> = ({ active, setActive }) => {
    ...
    const [dataBasket, setDataBasket] = useState<DataBasketElem[] | any[]>([]);
            ...
            <div onClick={e => e.stopPropagation()} className='basket-container__content'>
                ...
                {showBasketMenu ? <div>
                    ...
                    <div className='basket-container__make-an-order'>
                        ...
                        {dataBasket.length === 0 ? <></> : <NavLink onClick={() => setActive(false)} to={`/checkout`}><div className='basket-container__make-an-order__submit'><h2>ОФОРМИТИ ЗАМОВЛЕННЯ</h2></div></NavLink>}
                    </div>
                </div> :
